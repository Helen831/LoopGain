...
if isfield(settings,'AllowNoisierFlow') && settings.AllowNoisierFlow==1
    % 2021-11-02 10:45:35 Tuesday EAS changed from AllowNoisierFlow to AllowNoisierFlowFactor (bug fix)
    noisewav = FlowSignalToNoise(timewav,respwav,settings.plotfigure,settings.AllowNoisierFlowFactor); 
else
...
if (isfield(settings,'MouthBreathingDetect') && settings.MouthBreathingDetect==1)
    mouthBreathingData = zeros(length(winnumrange),6) * NaN;
end
...
for winNum=winnumrange
    ...
    if (isfield(settings,'MouthBreathingDetect') && settings.MouthBreathingDetect==1)
        ...
        [meanFlow, medianFlow, stdDevFlow, IQRFlow, percentDifference, mouthBreathing] = CalculateFlowMetrics(windowFlow);
        mouthBreathingData(winNum,:)=[meanFlow, medianFlow, stdDevFlow, IQRFlow, percentDifference, mouthBreathing];
    end
...
if (isfield(settings,'MouthBreathingDetect') && settings.MouthBreathingDetect==1)
    try
        ...
        mouthBreathingDataT.Properties.VariableNames = {'meanFlow', 'medianFlow', 'stdDevFlow', 'IQRFlow', 'percentDifference', 'mouthBreathing'};
